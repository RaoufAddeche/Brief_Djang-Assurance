...
def after_migrations(sender, **kwargs):
    ...
    if not Reg_model.objects.all():
        ...
        user1 = CustomUser.objects.create_user(
            username='JeanMichou', prenom='Jean', nom='Michou', password='password',
            age=72, adresse="2 rue de la corne", email="qqsdsd@sd.fr"
        )
        user2 = CustomUser.objects.create_user(
            username='gisèle', prenom='gis', nom='elle', password='password',
            age=19, adresse="37 rue de la licorne", email="qsd@sd.fr"
        )
        ...
        pred_list = []
        pred_list.append(Prediction.objects.create(
            user_id=user1, made_by=user3, age=30, reg_model=reg1, children=2, weight=70.5, size=170, smoker='no', region='southwest'
        ))
        pred_list.append(Prediction.objects.create(
            user_id=user2, made_by=user3, age=40, reg_model=reg2, children=1, weight=80.2, size=175, smoker='yes', region='northeast'
        ))
        pred_list.append(Prediction.objects.create(
            user_id=user2, made_by=user4, age=50, reg_model=reg3, children=0, weight=90.2, size=165, smoker='yes', region='southeast'
        ))
        pred_list.append(Prediction.objects.create(
            user_id=user1, made_by=user3, age=45, reg_model=reg4, children=4, weight=103, size=197, smoker='no', region='southwest'
        ))
        pred_list.append(Prediction.objects.create(
            user_id=user2, made_by=user3, age=70, reg_model=reg5, children=3, weight=150, size=145, smoker='no', region='northeast'
        ))
        pred_list.append(Prediction.objects.create(
            user_id=user1, made_by=user4, age=55, reg_model=reg6, children=5, weight=45, size=160, smoker='yes', region='southeast'
        ))
        ...
        Appointment.objects.create(
            user=user1, staff_user=staff2, date=date(2025, 1, 30), start_time=time(14, 0), end_time=time(15, 0)
        )
        Appointment.objects.create(
            user=user1, staff_user=staff1, date=date(2025, 1, 30), start_time=time(13, 0), end_time=time(14, 0)
        )
        Appointment.objects.create(
            user=user1, staff_user=staff3, date=date(2025, 1, 30), start_time=time(15, 0), end_time=time(16, 0)
        )
